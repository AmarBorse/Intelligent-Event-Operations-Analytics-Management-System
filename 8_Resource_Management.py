...
if role == "Admin":
    ...
    if st.button("Add Resource"):
        ...
        cursor.execute(
            "INSERT INTO resources (name,quantity,staff_role) VALUES (%s,%s,%s)",
        ...
        for r in requests:
            ...
            if r[5] == "Pending":
                if col6.button("Approve", key=r[0]):
                    ...
                    cursor.execute(
                        "UPDATE resource_requests SET status='Approved' WHERE id=%s",
                        (r[0],)
                    )
...
# ================= ORGANIZER PANEL =================
    ...
    cursor.execute(
        "SELECT id,name FROM events WHERE created_by=%s",
