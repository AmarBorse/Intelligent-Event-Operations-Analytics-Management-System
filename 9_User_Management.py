...
if search_user:
    ...
    cursor.execute(
        "SELECT id,username FROM users WHERE role='Organizer' AND username LIKE %s",
        ("%" + search_user + "%",)
    )
...
for org in organizers:
    ...
    with col2:
        if st.button("Delete", key=org[0]):
            ...
            cursor.execute("DELETE FROM users WHERE id=%s", (org[0],))
            conn.commit()
            conn.close()
...
for r in report:
    ...
    cursor.execute(
        "SELECT name FROM events WHERE created_by=%s",
        (r[0],)
    )
