...
for e in events:
    ...
    cursor.execute(
        "SELECT COUNT(*) FROM participants WHERE event_id=%s AND attended=1",
        (event_id,)
    )
    ...
    # ---------- BUDGET ----------
    cursor.execute(
        "SELECT SUM(amount) FROM expenses WHERE event_id=%s",
        (event_id,)
    )
    ...
    # ---------- FEEDBACK ----------
    cursor.execute(
        "SELECT AVG(rating) FROM feedback WHERE event_id=%s",
        (event_id,)
    )
...
for index,row in df.iterrows():
    ...
    if role == "Admin":
        if col7.button("Delete",key=row["Event ID"]):
            ...
            cursor.execute("DELETE FROM participants WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM expenses WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM feedback WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM staff_assignments WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM resource_allocations WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM resource_requests WHERE event_id=%s",(row["Event ID"],))
            cursor.execute("DELETE FROM staff_requests WHERE event_id=%s",(row["Event ID"],))

            cursor.execute("DELETE FROM events WHERE id=%s",(row["Event ID"],))
